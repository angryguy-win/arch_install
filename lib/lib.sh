...
# @description Function to read TOML file and update INSTALL_GROUPS
...
parse_stages_toml() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        case "$line" in
            '['*']')
                current_section=$(printf "%s" "$line" | sed -e 's/^\[//' -e 's/\]$//')
                ;;
            *)
                # Parse the line based on the current section
                case "$current_section" in
                    stages)
                        stage=$(printf "%s" "$line" | sed -n 's/^"\([^"]*\)".*/\1/p')
                        if [ -n "$stage" ]; then
                            content=$(printf "%s" "$line" | sed 's/^[^{]*{\(.*\)}/\1/')
                            mandatory_scripts=$(printf "%s" "$content" | sed -n 's/.*mandatory[[:space:]]*=[[:space:]]*\[\([^]]*\)\].*/\1/p')
                            optional_scripts=$(printf "%s" "$content" | sed -n 's/.*optional[[:space:]]*=[[:space:]]*\[\([^]]*\)\].*/\1/p')
                            if [ -n "$mandatory_scripts" ]; then
                                INSTALL_SCRIPTS["$stage,m"]=$(printf "%s" "$mandatory_scripts" | tr ',' ' ')
                            fi
                            if [ -n "$optional_scripts" ]; then
                                INSTALL_SCRIPTS["$stage,o"]=$(printf "%s" "$optional_scripts" | tr ',' ' ')
                            fi
                        fi
                        ;;
                    format_types|desktop_environments)
                        key=$(printf "%s" "$line" | sed -n 's/^\([^=]*\)[[:space:]]*=.*/\1/p' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
                        value=$(printf "%s" "$line" | sed -n 's/^[^=]*=[[:space:]]*\[\(.*\)\]/\1/p' | sed 's/"//g' | tr ',' ' ')
